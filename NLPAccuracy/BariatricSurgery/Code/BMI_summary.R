...
N.VAR=15
Variable_Name=c("Receiving bariatric surgery discussion within 1 year of study entry", 
                "Female", "Age at study entry","Race-White" , "Married", "BMI-Baseline",
                "English as Primary Language", "Nongovernment insurance","Median household income by zip code ($1,000)",
                "Charlson Co-morbidity Index","Diabetes",
                "Hypertension","Coronary artery disease","Stroke","Smoking")

####### Original Fitting Result ###################################################
Ori_fit<-lmer(PrimaryOutcome_BMIchange ~ Predictor_DiscBin_Project135ad +Female+ 
...
result=data.frame(variable_name=Variable_Name,estimates=-summary(Ori_fit)$coefficients[-1,c(1)],pvalue=format.pval(round(2*pnorm(abs(summary(Ori_fit)$coefficients[-1,c(1)]/summary(Ori_fit)$coefficients[-1,c(2)]),lower.tail=FALSE),digits = 4)) )
...
for(REP in 1:500){
  remove(fit)
  load(file=paste(Output_Path,"BMI_Sensitivity_",REP,".Rdata",sep=""))
  Output_Path="/data/cci_radiology/CCI/Alex Turchin/NLPAccuracy/BariatricSurgery/Output/"
  Sim_fit[[REP]]<-fit
}
################################################################################
...
for(v in 1:15 ){
  SimHR=data.frame(HR=sapply(1:500,function(REP) fixef(Sim_fit[[REP]])[v+1]),Original=fixef(Ori_fit)[v+1],Name=Variable_Name[v])
...
tiff(paste(Output_Path, "BMI.tiff",sep=""),width=900,height=900)
ggplot(HR_data, aes(y=HR,fill=Name)) + theme(
  axis.text.x=element_blank() ,
  #axis.title.x = element_text(color="blue", size=10, face="bold"),
  axis.text.y = element_text(size = 20),
  axis.title.y = element_text(color="#993333", size=20, face="bold")
)+geom_boxplot()+ylab("Fixed Effect")+geom_hline(aes(yintercept=Original), linetype="dashed", color = "red", size=1)+theme(legend.position = "none")+
...
for(v in 1:15){
  SimPvalue=data.frame(Pvalue=sapply(1:500,function(REP) 2*pnorm(abs(summary(Sim_fit[[REP]])$coefficients[v+1,c(1)]/summary(Sim_fit[[REP]])$coefficients[v+1,c(2)]),lower.tail=FALSE)),
                       Name=Variable_Name[v],
                       Original=2*pnorm(abs(summary(Ori_fit)$coefficients[v+1,c(1)]/summary(Ori_fit)$coefficients[v+1,c(2)]),lower.tail=FALSE))
...
write_xlsx(result,file=paste(Output_Path, "BMI_ori_table.xlsx",sep=""))
save(result,file=paste(Output_Path, "table_BMI.Rdata",sep=""))
################################################################################


# ##### Visualize if point-wise estimation is accurate ############################
# N.VAR=15
# Variable_Name=c("Discussion", "Female", "Age","Race-White" , "Married", "BMI-Baseline",
#                 "English as Primary Language", "Nongovernment insurance","Median House Income",
#                 "CCI","DM","HTN","CAD","CVA","Smoking")
# Plot_List=list()
# 
# for(v in 1:N.VAR){
#   
#   SimHR=data.frame(HR=sapply(1:500,function(REP)  -fixef(Sim_fit[[REP]])[v+1]),Name=Variable_Name[v])
...
#   )+
#     geom_boxplot()+xlab(Variable_Name[v])+ylab("Fixed Effect")+geom_hline(yintercept=-fixef(Ori_fit)[v+1], linetype="dashed", 
#                                                                           color = "red", size=2)
# }
# 
